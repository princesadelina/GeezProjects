...
@bot.on(geezbot_cmd(outgoing=True, pattern="itos$"))
async def _(event):
    if event.fwd_from:
        return
...
@bot.on(geezbot_cmd(outgoing=True, pattern="get$"))
async def _(event):
    if event.fwd_from:
        return
    if not event.reply_to_msg_id:
...
@bot.on(geezbot_cmd(outgoing=True, pattern="stoi$"))
