...
@bot.on(geezbot_cmd(outgoing=True, pattern=r"lock ?(.*)"))
...
@bot.on(geezbot_cmd(outgoing=True, pattern=r"unlock ?(.*)"))
...
CMD_HELP.update(
    {
        "locks": f">`{geez}lock <all (or) type(s)>` or >`{geez}unlock <all (or) type(s)>`"
