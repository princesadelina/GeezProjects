...
@bot.on(geezbot_cmd(outgoing=True, pattern=r"sendto ?(.*)"))
...
@bot.on(geezbot_cmd(outgoing=True, pattern=r"fwdto ?(.*)"))
async def catbroadcast_send(event):
    if event.fwd_from:
        return
    catinput_str = event.pattern_match.group(1)
...
@bot.on(geezbot_cmd(outgoing=True, pattern="addto ?(.*)"))
...
@bot.on(geezbot_cmd(outgoing=True, pattern="rmfrom ?(.*)"))
...
@bot.on(geezbot_cmd(outgoing=True, pattern="list ?(.*)"))
...
@bot.on(geezbot_cmd(outgoing=True, pattern="listall ?(.*)"))
async def catbroadcast_list(event):
    if event.fwd_from:
        return
...
@bot.on(geezbot_cmd(outgoing=True, pattern=r"frmfrom ?(.*)"))
async def catbroadcast_remove(event):
    if event.fwd_from:
        return
    catinput_str = event.pattern_match.group(1)
...
@bot.on(geezbot_cmd(outgoing=True, pattern="delc ?(.*)"))
