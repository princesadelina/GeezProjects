...
@bot.on(geezbot_cmd(outgoing=True, pattern=r"trump(?: |$)(.*)"))
...
@bot.on(geezbot_cmd(outgoing=True, pattern=r"qg(?: |$)(.*)"))
async def qg(event):
    text = event.pattern_match.group(1)
    text = re.sub("&", "", text)
    reply_to_id = event.message
...
@bot.on(geezbot_cmd(outgoing=True, pattern=r"cmm(?: |$)(.*)"))
async def cmm(event):
    text = event.pattern_match.group(1)
    text = re.sub("&", "", text)
    reply_to_id = event.message
...
@bot.on(geezbot_cmd(outgoing=True, pattern=r"kanna(?: |$)(.*)"))
...
@bot.on(geezbot_cmd(outgoing=True, pattern=r"tweet(?: |$)(.*)"))
...
CMD_HELP.update(
    {
        "nekobot": f">`{geez}tweet` <username>.<tweet>"
