...
@bot.on(geezbot_cmd(outgoing=True, pattern=r"stop (.*)"))
...
@bot.on(geezbot_cmd(outgoing=True, pattern="rmbotfilters (.*)"))
...
@bot.on(geezbot_cmd(outgoing=True, pattern="filters$"))
...
CMD_HELP.update({
    ...
    \n\n`{geez}filter` <keyword> <reply text> or reply to a message with .filter <keyword>\
