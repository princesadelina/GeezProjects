...
@bot.on(geezbot_cmd(outgoing=True, pattern=r"netease (?:(now)|(.*) - (.*))"))
async def _(event):
    if event.fwd_from:
        return
...
@bot.on(geezbot_cmd(outgoing=True, pattern=r"song(?: |$)(.*)"))
...
@bot.on(geezbot_cmd(outgoing=True, pattern=r"vsong(?: |$)(.*)"))
async def _(event):
...
@bot.on(geezbot_cmd(outgoing=True, pattern=r"smd(?: |$)(.*)"))
async def _(event):
    if event.fwd_from:
        return
...
@bot.on(geezbot_cmd(pattern=r"deez (.+?|) (FLAC|MP3\_320|MP3\_256|MP3\_128)"))
async def _(event):
    if event.fwd_from:
        return
