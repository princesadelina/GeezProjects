...
@bot.on(geezbot_cmd(outgoing=True, pattern="reserved$"))
...
@bot.on(geezbot_cmd(outgoing=True, pattern="name"))
...
@bot.on(geezbot_cmd(outgoing=True, pattern="setbio (.*)"))
...
@bot.on(geezbot_cmd(pattern="data(?: |$)(.*)", outgoing=True))
async def who(event):
...
CMD_HELP.update({
    "profile":
    f"`{geez}username` <new_username>\
\nUsage: Changes your Telegram username.\
\n\n`{geez}name` <firstname> or `.name` <firstname> <lastname>\
...
\n\n`{geez}delpfp` or `.delpfp` <number>/<all>\
...
\n\n`{geez}data` <username> or reply to someones text with `.data`\
