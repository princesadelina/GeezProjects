...
@bot.on(geezbot_cmd(outgoing=True, pattern=r"(?:colong|kang)\s?(.)?"))
async def kang(args):
...
@bot.on(geezbot_cmd(outgoing=True, pattern=r"stkrinfo$"))
...
@bot.on(geezbot_cmd(outgoing=True, pattern=r"getsticker$"))
...
CMD_HELP.update(
    {
        "stickers": f">`{geez}kang [emoji('s)]?`"
        f"\nUsage: Reply {geez}kang to a sticker or an image to kang it to your userbot pack "
        "\nor specify the emoji you want to."
        f"\n\n>`{geez}kang (emoji['s]]?` [number]?"
