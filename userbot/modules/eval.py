...
@bot.on(geezbot_cmd(outgoing=True, pattern=r"eval(?: |$)(.*)"))
...
@bot.on(geezbot_cmd(outgoing=True, pattern=r"exec(?: |$)([\s\S]*)"))
async def run(run_q):
...
@bot.on(geezbot_cmd(outgoing=True, pattern=r"term(?: |$)(.*)"))
...
CMD_HELP.update({"eval": f">`{geez}eval 2 + 3`"
                 "\nUsage: Evalute mini-expressions.",
                 f"exec": ">`{geez}exec print('hello')`"
                 "\nUsage: Execute small python scripts.",
                 f"term": ">`{geez}term <cmd>`"
